...
describe("ParentChildMultisigSetup", () => {
  ...
  const DENY_PROPOSAL_PERMISSION_ID = ethers.utils.id(
    "DENY_PROPOSAL_PERMISSION",
  );
  ...
  it("returns the plugin, helpers, and permissions", async () => {
    ...
    const adminConditionAddr = permissions[3].condition;
  ...
  it("sets up the plugin", async () => {
    ...
    await parentChildMultisigSetup.prepareInstallation(
      daoAddress,
      minimum_data,
    );

    const parentChildMultisigContract = ParentChildMultisigFactory.attach(
  ...
  it("correctly returns permissions for uninstallation", async () => {
    const plugin = ethers.Wallet.createRandom().address;

    const permissions = await parentChildMultisigSetup.callStatic
